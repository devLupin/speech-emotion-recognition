...
data_path = 'dataset/Actor_*/*.wav'

file_name = 'features+labels.npy'
...
model_name = 'cnn_to_transformer'
checkpoint_filename = f'./model/{model_name}.pkl'
print(f'[*] model: {model_name}')

device = 'cuda'

model = cnn_to_transformer(num_emotions=len(emotions_dict)).to(device) 
...
def preprocess_and_save():
    ...
    waveforms, emotions, intensities, genders = load_data(data_path, emotion_attributes)
    ...
    features_train , y_train = augment_waveforms(X_train, features_train, y_train, multiples, sample_rate)

    print('\n\nValidation waveforms:') # augment waveforms of validation set
    features_valid, y_valid = augment_waveforms(X_valid, features_valid, y_valid, multiples, sample_rate)

    print('\n\nTest waveforms:') # augment waveforms of test set 
    features_test, y_test = augment_waveforms(X_test, features_test, y_test, multiples, sample_rate)
    ...
    X_train, X_valid, X_test, y_train, y_valid, y_test = feature_scaling(X_train, X_valid, X_test, y_train, y_valid, y_test)
    ...
    print(f'Features and labels saved to {file_name}')
...
def main():
    ...
    # print(model)
    summary(model, input_size=(minibatch, 1,40,282))
    ...
    load_checkpoint(optimizer, model, checkpoint_filename)
